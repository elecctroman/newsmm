...
if (in_array('sqlite', PDO::getAvailableDrivers(), true)) {
    ...
    $cleanupPdo = new PDO('sqlite::memory:');
    $cleanupPdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $cleanupPdo->exec('CREATE TABLE hero_banners (id INTEGER PRIMARY KEY AUTOINCREMENT, side TEXT, image_url TEXT, link_url TEXT, alt_text TEXT, is_active INTEGER, sort_order INTEGER, starts_at TEXT, ends_at TEXT)');
    $cleanupPdo->exec('CREATE TABLE hero_slides (id INTEGER PRIMARY KEY AUTOINCREMENT, image_url TEXT, link_url TEXT, alt_text TEXT, is_active INTEGER, sort_order INTEGER, starts_at TEXT, ends_at TEXT)');
    $cleanupPdo->exec('CREATE TABLE hero_strip_items (id INTEGER PRIMARY KEY AUTOINCREMENT, image_url TEXT, title TEXT, link_url TEXT, is_active INTEGER, sort_order INTEGER)');
    $cleanupPdo->exec('CREATE TABLE settings (setting_key TEXT PRIMARY KEY, setting_value TEXT)');

    $cleanupPdo->prepare('INSERT INTO hero_banners (side, image_url, link_url, alt_text, is_active, sort_order, starts_at, ends_at) VALUES (:side, :image_url, :link_url, :alt_text, :is_active, :sort_order, :starts_at, :ends_at)')->execute([
        'side' => 'left',
    ...
    $cleanupPdo->prepare('INSERT INTO hero_slides (image_url, link_url, alt_text, is_active, sort_order, starts_at, ends_at) VALUES (:image_url, :link_url, :alt_text, :is_active, :sort_order, :starts_at, :ends_at)')->execute([
    ...
    $cleanupPdo->prepare('INSERT INTO hero_strip_items (image_url, title, link_url, is_active, sort_order) VALUES (:image_url, :title, :link_url, :is_active, :sort_order)')->execute([
    ...
    $cleanupPdo->prepare('INSERT INTO settings (setting_key, setting_value) VALUES (:key, :value)')->execute([
        'key' => 'hero_autoplay_enabled',
        'value' => '1',
    ]);

    archiveAndDropLegacyHeroTables($cleanupPdo);

    try {
        $cleanupPdo->query('SELECT COUNT(*) FROM hero_banners');
        expectEquals(true, false, 'hero_banners table should have been dropped');
    } catch (Throwable $expected) {
        // expected path on successful cleanup
    }

    $orphanStmt = $cleanupPdo->prepare('SELECT setting_value FROM settings WHERE setting_key = :key');
    $orphanStmt->execute(['key' => 'media_orphans_slider']);
    $orphanPayload = $orphanStmt->fetchColumn();
    if ($orphanPayload !== false) {
        expectEquals(true, str_contains((string) $orphanPayload, '/media/test-left.jpg'), 'archiveAndDropLegacyHeroTables stores orphan media payload when supported');
    }

    $heroSettingStmt = $cleanupPdo->prepare('SELECT COUNT(*) FROM settings WHERE setting_key LIKE :key');
    $heroSettingStmt->execute(['key' => 'hero_%']);
    expectEquals(0, (int) $heroSettingStmt->fetchColumn(), 'archiveAndDropLegacyHeroTables removes hero settings');
} else {
