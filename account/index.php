...
if (!isset($pdo) || !$pdo instanceof PDO) {
    ...
    $orderStats = fetchCustomerOrderStats($pdo, (int) $customer['id'], $customer['email'] ?? null);
    $allOrders = fetchCustomerOrders($pdo, (int) $customer['id'], $customer['email'] ?? null);
    $recentOrders = array_slice($allOrders, 0, 5);
    foreach ($allOrders as $order) {
        $totalSpent += (int) ($order['total_cents'] ?? 0);
    }

    $tickets = fetchCustomerTickets($pdo, (int) $customer['id']);
