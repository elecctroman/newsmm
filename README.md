...
Temel yardımcı fonksiyon doğrulamaları için aşağıdaki betiği kullanabilirsiniz:

```bash
php tests/run.php
```

Betiğin tüm kontrolleri başarıyla tamamlaması `src/helpers.php` içindeki para/slug yardımcılarının beklenen çıktıları ürettiğini doğrular.
