...
- `home_blocks`: Ana sayfadaki kategori vitrinleri ve özel bloklar.
...
- Ana sayfa hero/slider bileşeni kaldırıldı; vitrin içeriği yalnızca `home_blocks` kayıtları ve kategori vitrinleri üzerinden yönetilir.
- Eski slider tabloları `_backup_*` olarak yedeklenir ve kalan medya yolları `media_orphans_slider` ayarıyla raporlanır. Yönetim panelinde bu kayıtlar "Yetim slider görselleri" kartında görüntülenir.
- Vitrin bloklarında yaptığınız değişiklikler önbelleğe alınmış sayfaları otomatik olarak temizler; blok sıralaması `sort_order` alanına göre belirlenir.
