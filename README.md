...
- `hero_banners`, `hero_slides`, `hero_strip_items`, `home_blocks`: Yeni hero vitrin ve kategori blok bileşenleri.
...
- Yönetim panelinden **Yönetim → Vitrin → Hero** ekranına giderek sol/sağ banner, ana carousel ve mini şerit içeriklerini tek form üzerinden yönetebilirsiniz. Sürükle-bırak sıralama işlemleri değişiklikten sonra otomatik olarak önbelleği temizler.
- Vitrin verileri `hero_banners`, `hero_slides` ve `hero_strip_items` tablolarında saklanır. `ensureHeroInfrastructure` yardımcı fonksiyonu eski slider tablolarını `_backup_*` biçiminde yedekler.
- Müşteri tarafındaki carousel, ok/point kontrolleri ve hover/dokunmatik duraklatma ile 4 saniyelik otomatik geçiş destekler; erişilebilirlik için `aria-*` öznitelikleri ve klavye kısayolları uygulanmıştır.
- Dış istemciler için `GET /api/home/hero` uç noktası hero verilerini JSON olarak döndürür ve sonuç 60 saniye boyunca disk üzerinde önbelleğe alınır. Admin panelinde yapılan her CRUD işlemi bu önbelleği temizler.
