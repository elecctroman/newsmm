...
function fetchCustomerOrders(PDO $pdo, int $customerId, ?string $email = null): array
{
    ...
    $sql = 'SELECT o.id, o.order_no, o.status, o.total_cents, o.currency, o.notes, o.created_at, ' . $selectColumn
        . ' FROM orders o WHERE (o.customer_id = :customer_id';
    $params = ['customer_id' => $customerId];
    if ($email !== null && $email !== '') {
        $sql .= ' OR (o.customer_id IS NULL AND o.customer_email = :email)';
...
function fetchCustomerOrderStats(PDO $pdo, int $customerId, ?string $email = null): array
{
    ...
    $params = ['customer_id' => $customerId];
    if ($email !== null && $email !== '') {
        $sql .= ' OR (customer_id IS NULL AND customer_email = :email)';
        $params['email'] = trim(strtolower($email));
    }
    $sql .= ') GROUP BY status';
