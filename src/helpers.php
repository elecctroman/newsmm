...
function ensureCategoryInfrastructure(PDO $pdo): void
{
    ...
    try {
        ...
        // ignore setup failures, individual queries below will guard themselves
    }

    if (!tableHasColumn($pdo, 'categories', 'icon')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN icon VARCHAR(120) DEFAULT NULL AFTER emoji');
            tableHasColumn($pdo, 'categories', 'icon', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }

    if (!tableHasColumn($pdo, 'category_subcategories', 'icon')) {
        try {
            $pdo->exec('ALTER TABLE category_subcategories ADD COLUMN icon VARCHAR(120) DEFAULT NULL AFTER description');
            tableHasColumn($pdo, 'category_subcategories', 'icon', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }

    if (!tableHasColumn($pdo, 'category_subcategories', 'description')) {
        try {
            $pdo->exec('ALTER TABLE category_subcategories ADD COLUMN description VARCHAR(255) DEFAULT "" AFTER title');
            tableHasColumn($pdo, 'category_subcategories', 'description', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }
    ...
    if (!tableHasColumn($pdo, 'products', 'subcategory_id')) {
        ...
        try {
            $pdo->exec('ALTER TABLE products ADD CONSTRAINT fk_products_subcategory FOREIGN KEY (subcategory_id) REFERENCES category_subcategories(id) ON DELETE SET NULL');
        } catch (Throwable $ignored) {
            // ignore
        }
    }
