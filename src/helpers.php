...
{
    archiveAndDropLegacyHeroTables($pdo);
...
function archiveAndDropLegacyHeroTables(PDO $pdo): void
{
    ...
    $tables = [
        'hero_banners' => ['image_column' => 'image_url', 'title_column' => 'alt_text'],
        'hero_slides' => ['image_column' => 'image_url', 'title_column' => 'alt_text'],
        'hero_strip_items' => ['image_column' => 'image_url', 'title_column' => 'title'],
    ];
    ...
    foreach ($tables as $table => $meta) {
        ...
        try {
            $stmt = $pdo->query('SELECT id, ' . $meta['image_column'] . ' AS image_url, ' . $meta['title_column'] . ' AS title FROM `' . $table . '`');
            if ($stmt !== false) {
        ...
        $backupTable = '_backup_' . $table;
        try {
            $pdo->exec('CREATE TABLE IF NOT EXISTS `' . $backupTable . '` LIKE `' . $table . '`');
        } catch (Throwable $ignored) {
            // ignore backup creation failures
        }

        try {
            $backupCountStmt = $pdo->query('SELECT COUNT(*) FROM `' . $backupTable . '`');
            $backupIsEmpty = $backupCountStmt !== false ? ((int) $backupCountStmt->fetchColumn() === 0) : true;
            if ($backupIsEmpty) {
        ...
        tableExists($pdo, $table, true);
    }

    if (!empty($orphanMedia) && tableExists($pdo, 'settings')) {
        $payload = json_encode(array_values($orphanMedia), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    ...
    if (tableExists($pdo, 'settings')) {
        try {
            $stmt = $pdo->prepare('DELETE FROM settings WHERE setting_key IN ("hero_autoplay_enabled","hero_autoplay_ms","hero_show_dots","hero_show_arrows")');
            $stmt->execute();
        } catch (Throwable $ignored) {
