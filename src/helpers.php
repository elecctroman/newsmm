...
function tableExists(PDO $pdo, string $table, bool $refresh = false): bool
{
    static $cache = [];
    $normalized = strtolower($table);
    if (!$refresh && array_key_exists($normalized, $cache)) {
    ...
    return $cache[$normalized] = $exists;
}
...
function ensureCategoryInfrastructure(PDO $pdo): void
{
    ...
    if (!tableHasColumn($pdo, 'categories', 'hero_image')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN hero_image VARCHAR(255) DEFAULT NULL');
            tableHasColumn($pdo, 'categories', 'hero_image', true);
        } catch (Throwable $ignored) {
            // ignore legacy environments without permissions
        }
    }
    ...
    if (!tableHasColumn($pdo, 'categories', 'sort_order')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN sort_order INT NOT NULL DEFAULT 0');
            tableHasColumn($pdo, 'categories', 'sort_order', true);
        } catch (Throwable $ignored) {
            // ignore legacy environments without permissions
        }
    }
    ...
    if (!tableHasColumn($pdo, 'categories', 'is_featured')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN is_featured TINYINT(1) NOT NULL DEFAULT 0');
            tableHasColumn($pdo, 'categories', 'is_featured', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }

    if (!tableHasColumn($pdo, 'categories', 'status')) {
        try {
            $pdo->exec("ALTER TABLE categories ADD COLUMN status ENUM('draft','review','published','archived') NOT NULL DEFAULT 'published'");
            tableHasColumn($pdo, 'categories', 'status', true);
        } catch (Throwable $ignored) {
            // ignore legacy environments without permissions
        }
    }

    if (!tableHasColumn($pdo, 'categories', 'published_at')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN published_at TIMESTAMP NULL DEFAULT NULL');
            tableHasColumn($pdo, 'categories', 'published_at', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }

    if (!tableHasColumn($pdo, 'categories', 'meta_title')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN meta_title VARCHAR(160) DEFAULT NULL');
            tableHasColumn($pdo, 'categories', 'meta_title', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }

    if (!tableHasColumn($pdo, 'categories', 'meta_description')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD COLUMN meta_description VARCHAR(320) DEFAULT NULL');
            tableHasColumn($pdo, 'categories', 'meta_description', true);
        } catch (Throwable $ignored) {
            // ignore
        }
    }
    ...
    if (tableHasColumn($pdo, 'categories', 'status')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD INDEX idx_categories_status (status)');
        } catch (Throwable $ignored) {
            // ignore duplicate index errors
        }
    }
    ...
    if (tableHasColumn($pdo, 'categories', 'is_featured')) {
        try {
            $pdo->exec('ALTER TABLE categories ADD INDEX idx_categories_featured (is_featured)');
        } catch (Throwable $ignored) {
            // ignore duplicate index errors
        }
    }
    ...
    if (tableHasColumn($pdo, 'categories', 'status')) {
        try {
            $pdo->exec("UPDATE categories SET status = 'published' WHERE status IS NULL OR status = ''");
        } catch (Throwable $ignored) {
            // ignore data updates on restricted environments
        }
    }
...
function getHeroCachePath(): string
{
    static $cachePath = '';
    if ($cachePath !== '') {
        return $cachePath;
    ...
    $cachePath = $directory . DIRECTORY_SEPARATOR . 'lisansonay_hero_cache.json';
    return $cachePath;
}

function loadHeroCache(int $ttlSeconds = 60): ?array
{
    $path = getHeroCachePath();
    if (!is_file($path)) {
        return null;
    }

    $lastModified = @filemtime($path);
    if (!$lastModified || (time() - $lastModified) > $ttlSeconds) {
        return null;
    }
...
function saveHeroCache(array $payload): void
{
    $path = getHeroCachePath();
...
function ensureHeroInfrastructure(PDO $pdo): void
{
    static $ensured = false;
    if ($ensured) {
        return;
    ...
    $legacyTables = [
        ...
        'home_slider',
        'home_slider_old',
        'home_strip_items_old',
    ];

    foreach ($legacyTables as $legacyTable) {
        if (!tableExists($pdo, $legacyTable)) {
            continue;
        }

        $backupTable = '_backup_' . $legacyTable;
        try {
            $pdo->exec('CREATE TABLE IF NOT EXISTS `' . $backupTable . '` LIKE `' . $legacyTable . '`');
            $pdo->exec('INSERT IGNORE INTO `' . $backupTable . '` SELECT * FROM `' . $legacyTable . '`');
        } catch (Throwable $ignored) {
            // continue even if backup copy fails
        }

        try {
            $pdo->exec('DROP TABLE IF EXISTS `' . $legacyTable . '`');
        } catch (Throwable $ignored) {
            // ignore drop failures
        }

        tableExists($pdo, $legacyTable, true);
    }
    ...
    try {
        $pdo->exec('CREATE TABLE IF NOT EXISTS hero_banners (
            ...
            INDEX idx_hero_banners_side (side),
            INDEX idx_hero_banners_active (is_active, sort_order)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');
    } catch (Throwable $ignored) {
        // ignore
    }
    ...
    try {
        $pdo->exec('CREATE TABLE IF NOT EXISTS hero_slides (
            ...
            created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            INDEX idx_hero_slides_active (is_active, sort_order)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');
    } catch (Throwable $ignored) {
        // ignore
    }
    ...
    try {
        $pdo->exec('CREATE TABLE IF NOT EXISTS hero_strip_items (
            id INT AUTO_INCREMENT PRIMARY KEY,
            image_url VARCHAR(512) NOT NULL,
            ...
            created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            INDEX idx_hero_strip_active (is_active, sort_order)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');
    } catch (Throwable $ignored) {
        // ignore
    }

    tableExists($pdo, 'hero_banners', true);
    tableExists($pdo, 'hero_slides', true);
    tableExists($pdo, 'hero_strip_items', true);

    try {
        $bannerCount = 0;
        $stmt = $pdo->query('SELECT COUNT(*) FROM hero_banners');
        if ($stmt !== false) {
            $bannerCount = (int) $stmt->fetchColumn();
        }
        if ($bannerCount === 0) {
            $pdo->exec("INSERT INTO hero_banners (side, image_url, link_url, alt_text, sort_order) VALUES
                ('left','/media/hero/left.jpg','/kategori/valorant','Valorant VP',10),
                ('right','/media/hero/right.jpg','/kategori/mobile-legends','Mobile Legends',10)");
        }
    } catch (Throwable $ignored) {
        // seeding best-effort
    }
    ...
    try {
        ...
        if ($stmt !== false) {
            $slideCount = (int) $stmt->fetchColumn();
        }
        if ($slideCount === 0) {
            $pdo->exec("INSERT INTO hero_slides (image_url, link_url, alt_text, sort_order) VALUES
                ('/media/hero/slide-1.jpg','/kampanya/poncik','Kampanya',10),
                ('/media/hero/slide-2.jpg','/kategori/valorant','Valorant',20),
                ('/media/hero/slide-3.jpg','/kategori/pubg','PUBG',30)");
        }
    } catch (Throwable $ignored) {
    ...
    try {
        $stripCount = 0;
        $stmt = $pdo->query('SELECT COUNT(*) FROM hero_strip_items');
        if ($stmt !== false) {
            $stripCount = (int) $stmt->fetchColumn();
        }
        if ($stripCount === 0) {
            $pdo->exec("INSERT INTO hero_strip_items (image_url, title, link_url, sort_order) VALUES
                ('/media/strip-wartune.jpg','Wartune','/kategori/wartune',10),
                ('/media/strip-cs2.jpg','Counter-Strike 2','/kategori/cs2',20),
                ('/media/strip-lol.jpg','League of Legends','/kategori/lol',30),
                ('/media/strip-pubg.jpg','PUBG Mobile','/kategori/pubg',40)");
        }
    } catch (Throwable $ignored) {
        // ignore
    }
