...
function fetchHomeSliderItems(PDO $pdo): array
{
    $stmt = $pdo->query('SELECT id, image_url, headline, link_url, starts_at, ends_at, sort_order, is_active FROM home_slider_items ORDER BY sort_order ASC, id ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

function createHomeSliderItem(PDO $pdo, array $payload): int
{
    $stmt = $pdo->prepare('INSERT INTO home_slider_items (image_url, headline, link_url, starts_at, ends_at, sort_order, is_active) VALUES (:image_url, :headline, :link_url, :starts_at, :ends_at, :sort_order, :is_active)');
    $stmt->execute([
        'image_url' => sanitizeNullableString($payload['image_url'] ?? ''),
        'headline' => sanitizeNullableString($payload['headline'] ?? null),
...
function updateHomeSliderItem(PDO $pdo, int $id, array $payload): void
{
    $stmt = $pdo->prepare('UPDATE home_slider_items SET image_url = :image_url, headline = :headline, link_url = :link_url, starts_at = :starts_at, ends_at = :ends_at, sort_order = :sort_order, is_active = :is_active WHERE id = :id');
    $stmt->execute([
        'image_url' => sanitizeNullableString($payload['image_url'] ?? ''),
        'headline' => sanitizeNullableString($payload['headline'] ?? null),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
...
function deleteHomeSliderItem(PDO $pdo, int $id): void
{
    $stmt = $pdo->prepare('DELETE FROM home_slider_items WHERE id = :id');
    $stmt->execute(['id' => $id]);
}

function fetchHomeStripItems(PDO $pdo): array
{
    $stmt = $pdo->query('SELECT id, name, description, image_url, link_url, sort_order, is_active FROM home_strip_items ORDER BY sort_order ASC, id ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

function createHomeStripItem(PDO $pdo, array $payload): int
{
    $stmt = $pdo->prepare('INSERT INTO home_strip_items (name, description, image_url, link_url, sort_order, is_active) VALUES (:name, :description, :image_url, :link_url, :sort_order, :is_active)');
    $stmt->execute([
        'name' => trim((string) ($payload['name'] ?? '')),
        'description' => sanitizeNullableString($payload['description'] ?? null),
        'image_url' => sanitizeNullableString($payload['image_url'] ?? null),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
        'sort_order' => (int) ($payload['sort_order'] ?? 0),
        'is_active' => isset($payload['is_active']) ? ((bool) $payload['is_active'] ? 1 : 0) : 1,
    ]);
...
function updateHomeStripItem(PDO $pdo, int $id, array $payload): void
{
    $stmt = $pdo->prepare('UPDATE home_strip_items SET name = :name, description = :description, image_url = :image_url, link_url = :link_url, sort_order = :sort_order, is_active = :is_active WHERE id = :id');
    $stmt->execute([
        'name' => trim((string) ($payload['name'] ?? '')),
        'description' => sanitizeNullableString($payload['description'] ?? null),
        'image_url' => sanitizeNullableString($payload['image_url'] ?? null),
...
function deleteHomeStripItem(PDO $pdo, int $id): void
{
    $stmt = $pdo->prepare('DELETE FROM home_strip_items WHERE id = :id');
    $stmt->execute(['id' => $id]);
}
