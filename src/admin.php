...
function fetchCategoryTree(PDO $pdo): array
{
    ...
    $select = 'SELECT id, slug, title, description, emoji, icon, sort_order, created_at, ';
    ...
    if (empty($categories) || !tableHasColumn($pdo, 'category_subcategories', 'id')) {
        return array_values($categories);
    }

    $hasSubUpdated = tableHasColumn($pdo, 'category_subcategories', 'updated_at');
    $subSelect = 'SELECT id, category_id, slug, title, description, icon, sort_order, created_at, ';
    $subSelect .= $hasSubUpdated ? 'updated_at' : 'created_at AS updated_at';
    $subSelect .= ' FROM category_subcategories ORDER BY sort_order ASC, title ASC';

    $subStmt = $pdo->query($subSelect);
...
{
    ensureCategoryInfrastructure($pdo);
    $stmt = $pdo->prepare('SELECT id, slug, title, description, emoji, icon, sort_order FROM categories WHERE id = :id');
    $stmt->execute(['id' => $id]);
    $category = $stmt->fetch(PDO::FETCH_ASSOC);
    return $category ?: null;
}

function fetchSubcategoryById(PDO $pdo, int $id): ?array
{
    ensureCategoryInfrastructure($pdo);
    if (!tableHasColumn($pdo, 'category_subcategories', 'id')) {
        return null;
    }
    $stmt = $pdo->prepare('SELECT id, category_id, slug, title, description, icon, sort_order FROM category_subcategories WHERE id = :id');
    $stmt->execute(['id' => $id]);
    $subcategory = $stmt->fetch(PDO::FETCH_ASSOC);
    return $subcategory ?: null;
}

function fetchSubcategories(PDO $pdo, ?int $categoryId = null): array
{
    ensureCategoryInfrastructure($pdo);
    if (!tableHasColumn($pdo, 'category_subcategories', 'id')) {
        return [];
    ...
    $hasUpdated = tableHasColumn($pdo, 'category_subcategories', 'updated_at');
    $select = 'SELECT id, category_id, slug, title, description, icon, sort_order, created_at, ';
    $select .= $hasUpdated ? 'updated_at' : 'created_at AS updated_at';
    $select .= ' FROM category_subcategories';
    $params = [];
    ...
    $select .= ' ORDER BY sort_order ASC, title ASC';
...
{
    ensureCategoryInfrastructure($pdo);
    $stmt = $pdo->prepare('INSERT INTO categories (slug, title, description, emoji, icon, sort_order) VALUES (:slug, :title, :description, :emoji, :icon, :sort_order)');
    $stmt->execute([
        'slug' => sanitizeSlug($payload['slug'] ?? ''),
        'title' => trim($payload['title'] ?? ''),
        'description' => trim($payload['description'] ?? ''),
        'emoji' => trim($payload['emoji'] ?? ''),
        'icon' => sanitizeNullableString($payload['icon'] ?? null),
...
{
    ensureCategoryInfrastructure($pdo);
    $stmt = $pdo->prepare('UPDATE categories SET slug = :slug, title = :title, description = :description, emoji = :emoji, icon = :icon, sort_order = :sort_order WHERE id = :id');
    $stmt->execute([
        'slug' => sanitizeSlug($payload['slug'] ?? ''),
        'title' => trim($payload['title'] ?? ''),
        'description' => trim($payload['description'] ?? ''),
...
function createSubcategory(PDO $pdo, array $payload): int
{
    ensureCategoryInfrastructure($pdo);
    if (!tableHasColumn($pdo, 'category_subcategories', 'id')) {
        throw new RuntimeException('Alt kategori desteği etkinleştirilemedi.');
    }

    $stmt = $pdo->prepare('INSERT INTO category_subcategories (category_id, slug, title, description, icon, sort_order) VALUES (:category_id, :slug, :title, :description, :icon, :sort_order)');
    $stmt->execute([
...
function updateSubcategory(PDO $pdo, int $id, array $payload): void
{
    ensureCategoryInfrastructure($pdo);
    if (!tableHasColumn($pdo, 'category_subcategories', 'id')) {
        throw new RuntimeException('Alt kategori desteği etkinleştirilemedi.');
    ...
    $stmt = $pdo->prepare('UPDATE category_subcategories SET category_id = :category_id, slug = :slug, title = :title, description = :description, icon = :icon, sort_order = :sort_order WHERE id = :id');
    $stmt->execute([
        'category_id' => (int) ($payload['category_id'] ?? 0),
        'slug' => sanitizeSlug($payload['slug'] ?? ''),
        'title' => trim($payload['title'] ?? ''),
...
function deleteSubcategory(PDO $pdo, int $id): void
{
    ensureCategoryInfrastructure($pdo);
    if (!tableHasColumn($pdo, 'category_subcategories', 'id')) {
        return;
    }
    $stmt = $pdo->prepare('DELETE FROM category_subcategories WHERE id = :id');
    $stmt->execute(['id' => $id]);
}
...
function fetchAllProducts(PDO $pdo): array
{
    ...
    $select .= $hasUpdated ? 'p.updated_at' : 'p.created_at AS updated_at';
    $select .= ', c.title AS category_title, c.slug AS category_slug, c.id AS category_id, c.icon AS category_icon';
    if ($hasSubcategory) {
        $select .= ', p.subcategory_id, sc.title AS subcategory_title, sc.slug AS subcategory_slug, sc.icon AS subcategory_icon';
    } else {
        $select .= ', NULL AS subcategory_id, NULL AS subcategory_title, NULL AS subcategory_slug, NULL AS subcategory_icon';
    }
    $select .= ' FROM products p INNER JOIN categories c ON c.id = p.category_id';
    if ($hasSubcategory) {
        $select .= ' LEFT JOIN category_subcategories sc ON sc.id = p.subcategory_id';
    }
    $select .= ' ORDER BY c.sort_order ASC, c.title ASC, p.sort_order ASC, p.name ASC';
...
function createProduct(PDO $pdo, array $payload): int
{
    ...
    $stmt = $pdo->prepare('INSERT INTO products (category_id, subcategory_id, name, price_label, price_cents, note, sort_order) VALUES (:category_id, :subcategory_id, :name, :price_label, :price_cents, :note, :sort_order)');
    $stmt->execute([
...
function updateProduct(PDO $pdo, int $id, array $payload): void
{
    ...
    $stmt = $pdo->prepare('UPDATE products SET category_id = :category_id, subcategory_id = :subcategory_id, name = :name, price_label = :price_label, price_cents = :price_cents, note = :note, sort_order = :sort_order WHERE id = :id');
    $stmt->execute([
        'category_id' => (int)$payload['category_id'],
        'subcategory_id' => $subcategoryId,
        'name' => trim($payload['name'] ?? ''),
