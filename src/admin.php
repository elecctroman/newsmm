...
function fetchHeroBanners(PDO $pdo): array
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->query('SELECT id, side, image_url, link_url, alt_text, sort_order, is_active, starts_at, ends_at FROM hero_banners ORDER BY side ASC, sort_order ASC, id ASC');
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    $banners = [
        'left' => [],
        'right' => [],
...
function createHeroBanner(PDO $pdo, array $payload): int
{
    ensureHeroInfrastructure($pdo);
    $side = strtolower((string) ($payload['side'] ?? 'left')) === 'right' ? 'right' : 'left';
    $stmt = $pdo->prepare('INSERT INTO hero_banners (side, image_url, link_url, alt_text, sort_order, is_active, starts_at, ends_at) VALUES (:side, :image_url, :link_url, :alt_text, :sort_order, :is_active, :starts_at, :ends_at)');
    $stmt->execute([
        'side' => $side,
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
        'alt_text' => sanitizeNullableString($payload['alt_text'] ?? null),
        'sort_order' => (int) ($payload['sort_order'] ?? 0),
        'is_active' => isset($payload['is_active']) ? ((bool) $payload['is_active'] ? 1 : 0) : 1,
        'starts_at' => normalizeDateTimeInput($payload['starts_at'] ?? null),
        'ends_at' => normalizeDateTimeInput($payload['ends_at'] ?? null),
    ]);
    purgeHeroCache();
    return (int) $pdo->lastInsertId();
}

function updateHeroBanner(PDO $pdo, int $id, array $payload): void
{
    ensureHeroInfrastructure($pdo);
    $side = strtolower((string) ($payload['side'] ?? 'left')) === 'right' ? 'right' : 'left';
    $stmt = $pdo->prepare('UPDATE hero_banners SET side = :side, image_url = :image_url, link_url = :link_url, alt_text = :alt_text, sort_order = :sort_order, is_active = :is_active, starts_at = :starts_at, ends_at = :ends_at WHERE id = :id');
    $stmt->execute([
        'side' => $side,
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
...
function deleteHeroBanner(PDO $pdo, int $id): void
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('DELETE FROM hero_banners WHERE id = :id');
    $stmt->execute(['id' => $id]);
    purgeHeroCache();
}

function reorderHeroBanners(PDO $pdo, string $side, array $orderedIds): void
{
    ensureHeroInfrastructure($pdo);
    $cleanSide = strtolower($side) === 'right' ? 'right' : 'left';
    $order = 0;
    $stmt = $pdo->prepare('UPDATE hero_banners SET sort_order = :sort_order WHERE id = :id AND side = :side');
...
function fetchHeroSlides(PDO $pdo): array
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->query('SELECT id, image_url, link_url, alt_text, sort_order, is_active, starts_at, ends_at FROM hero_slides ORDER BY sort_order ASC, id ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
}

function createHeroSlide(PDO $pdo, array $payload): int
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('INSERT INTO hero_slides (image_url, link_url, alt_text, sort_order, is_active, starts_at, ends_at) VALUES (:image_url, :link_url, :alt_text, :sort_order, :is_active, :starts_at, :ends_at)');
    $stmt->execute([
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
        'alt_text' => sanitizeNullableString($payload['alt_text'] ?? null),
...
function updateHeroSlide(PDO $pdo, int $id, array $payload): void
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('UPDATE hero_slides SET image_url = :image_url, link_url = :link_url, alt_text = :alt_text, sort_order = :sort_order, is_active = :is_active, starts_at = :starts_at, ends_at = :ends_at WHERE id = :id');
    $stmt->execute([
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
        'link_url' => sanitizeNullableString($payload['link_url'] ?? null),
        'alt_text' => sanitizeNullableString($payload['alt_text'] ?? null),
...
function deleteHeroSlide(PDO $pdo, int $id): void
{
    ensureHeroInfrastructure($pdo);
...
function reorderHeroSlides(PDO $pdo, array $orderedIds): void
{
    ensureHeroInfrastructure($pdo);
...
function fetchHeroStripItems(PDO $pdo): array
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->query('SELECT id, image_url, title, link_url, sort_order, is_active FROM hero_strip_items ORDER BY sort_order ASC, id ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
}

function createHeroStripItem(PDO $pdo, array $payload): int
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('INSERT INTO hero_strip_items (image_url, title, link_url, sort_order, is_active) VALUES (:image_url, :title, :link_url, :sort_order, :is_active)');
    $stmt->execute([
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
...
function updateHeroStripItem(PDO $pdo, int $id, array $payload): void
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('UPDATE hero_strip_items SET image_url = :image_url, title = :title, link_url = :link_url, sort_order = :sort_order, is_active = :is_active WHERE id = :id');
    $stmt->execute([
        'image_url' => trim((string) ($payload['image_url'] ?? '')),
        'title' => trim((string) ($payload['title'] ?? '')),
        'link_url' => trim((string) ($payload['link_url'] ?? '')),
...
function deleteHeroStripItem(PDO $pdo, int $id): void
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('DELETE FROM hero_strip_items WHERE id = :id');
    $stmt->execute(['id' => $id]);
    purgeHeroCache();
}

function reorderHeroStripItems(PDO $pdo, array $orderedIds): void
{
    ensureHeroInfrastructure($pdo);
    $stmt = $pdo->prepare('UPDATE hero_strip_items SET sort_order = :sort_order WHERE id = :id');
    $order = 0;
    foreach ($orderedIds as $id) {
        $id = (int) $id;
        if ($id <= 0) {
