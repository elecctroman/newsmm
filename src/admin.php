...
function fetchAllProducts(PDO $pdo): array
{
    ...
    $select .= $hasUpdated ? 'p.updated_at' : 'p.created_at AS updated_at';
    $select .= ', c.name AS category_title, c.slug AS category_slug, c.id AS category_id, c.icon_url AS category_icon';
    if ($hasSubcategory) {
        $select .= ', p.subcategory_id, sc.name AS subcategory_title, sc.slug AS subcategory_slug, sc.icon_url AS subcategory_icon';
    } else {
