...
{
    $stmt = $pdo->prepare('SELECT p.id, p.name, p.price_label, p.price_cents, p.updated_at, c.title AS category_title FROM products p INNER JOIN categories c ON c.id = p.category_id ORDER BY p.updated_at DESC LIMIT :limit');
...
{
    $stmt = $pdo->prepare('SELECT id, order_no, customer_name, status, total_cents, currency, updated_at FROM orders ORDER BY updated_at DESC LIMIT :limit');
    $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
{
    $stmt = $pdo->query('SELECT id, slug, title, description, emoji, sort_order, created_at, updated_at FROM categories ORDER BY sort_order ASC, title ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
{
    $stmt = $pdo->query('SELECT p.id, p.name, p.price_label, p.price_cents, p.note, p.sort_order, p.created_at, p.updated_at, c.title AS category_title, c.slug AS category_slug, c.id AS category_id FROM products p INNER JOIN categories c ON c.id = p.category_id ORDER BY c.sort_order ASC, p.sort_order ASC, p.name ASC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
{
    $stmt = $pdo->query('SELECT id, order_no, customer_name, customer_email, customer_phone, status, total_cents, currency, notes, created_at, updated_at FROM orders ORDER BY created_at DESC');
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
