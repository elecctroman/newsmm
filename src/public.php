...
function fetchHeroLayout(PDO $pdo, array $settings = []): array
{
    ensureHeroInfrastructure($pdo);

    $now = date('Y-m-d H:i:s');
    $bannerTimeFilters = ' AND (starts_at IS NULL OR starts_at <= :banner_now_start)'
        . ' AND (ends_at IS NULL OR ends_at >= :banner_now_end)';
    $slideTimeFilters = ' AND (starts_at IS NULL OR starts_at <= :slide_now_start)'
        . ' AND (ends_at IS NULL OR ends_at >= :slide_now_end)';

    $bannerSql = 'SELECT id, side, image_url, link_url, alt_text, is_active, sort_order, starts_at, ends_at'
        . ' FROM hero_banners WHERE is_active = 1' . $bannerTimeFilters
        . ' ORDER BY sort_order ASC, id ASC';
    $bannerStmt = $pdo->prepare($bannerSql);
    $bannerStmt->execute([
        'banner_now_start' => $now,
        'banner_now_end' => $now,
    ]);
    ...
    foreach ($bannerRows as $banner) {
        ...
        $normalized = [
            ...
            'link_url' => sanitizeNullableString($banner['link_url'] ?? null),
            'alt_text' => sanitizeNullableString($banner['alt_text'] ?? null),
        ];
        if (($banner['side'] ?? '') === 'right') {
            if ($rightBanner === null) {
                $rightBanner = $normalized;
            }
            $rightBanners[] = $normalized;
        } else {
            if ($leftBanner === null) {
                $leftBanner = $normalized;
            }
            $leftBanners[] = $normalized;
        }
    }

    $slideSql = 'SELECT id, image_url, link_url, alt_text FROM hero_slides WHERE is_active = 1'
        . $slideTimeFilters . ' ORDER BY sort_order ASC, id ASC';
    ...
    $slides = [];
    foreach ($slideStmt->fetchAll(PDO::FETCH_ASSOC) ?: [] as $slide) {
        ...
        $slides[] = [
            ...
            'link_url' => sanitizeNullableString($slide['link_url'] ?? null),
            'alt_text' => sanitizeNullableString($slide['alt_text'] ?? null),
        ];
    }

    $stripStmt = $pdo->query('SELECT id, image_url, title, link_url FROM hero_strip_items WHERE is_active = 1 ORDER BY sort_order ASC, id ASC');
    $stripItems = [];
    foreach ($stripStmt->fetchAll(PDO::FETCH_ASSOC) ?: [] as $strip) {
        if (empty($strip['image_url']) || empty($strip['title']) || empty($strip['link_url'])) {
            continue;
        }
        $stripItems[] = [
            'id' => (int) $strip['id'],
            ...
            'link_url' => (string) $strip['link_url'],
        ];
    }

    $autoplayEnabled = !empty($settings['hero_autoplay_enabled']);
    $autoplayMs = isset($settings['hero_autoplay_ms']) ? (int) $settings['hero_autoplay_ms'] : 4000;
    $showDots = !empty($settings['hero_show_dots']);
    $showArrows = !empty($settings['hero_show_arrows']);
    ...
    return [
        ...
        'left_banners' => $leftBanners,
        'right_banners' => $rightBanners,
        'slides' => $slides,
        'strip' => $stripItems,
        'settings' => [
            'autoplay' => $autoplayEnabled,
            'autoplay_ms' => max(2000, $autoplayMs ?: 4000),
            'show_dots' => $showDots,
