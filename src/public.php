...
function fetchHomeSliderForPublic(PDO $pdo): array
{
    $now = date('Y-m-d H:i:s');
    $sql = 'SELECT id, image_url, headline, link_url FROM home_slider_items WHERE is_active = 1'
...
function fetchHomeStripForPublic(PDO $pdo): array
{
    $stmt = $pdo->query('SELECT id, name, description, image_url, link_url FROM home_strip_items WHERE is_active = 1 ORDER BY sort_order ASC, id ASC');
