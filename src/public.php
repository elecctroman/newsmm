...
function fetchPublicCatalog(PDO $pdo): array
{
    ...
    if ($hasSubcategory) {
        ...
        $select .= ' ORDER BY c.sort_order ASC, c.title ASC, sc.sort_order ASC, sc.title ASC, p.sort_order ASC, p.name ASC';
    } else {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($subcategoryId > 0 && isset($subcategoryIndex[$subcategoryId])) {
            ...
            if (isset($catalog[$parentCategoryId]['subcategories'][$subcategoryId])) {
